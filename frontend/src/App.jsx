...
import { checkHealth, queryAgentic, scrapeUrl, resetMemory } from './lib/api'
...
export default function App() {
  ...
  const handleInputChange = (e) => {
    ...
    // Auto-resize logic to prevent scrolling until max height
    e.target.style.height = 'auto'
    e.target.style.height = `${Math.min(e.target.scrollHeight, 208)}px`
    e.target.style.overflowY = e.target.scrollHeight > 208 ? 'auto' : 'hidden'
  }
  ...
  const handleQuery = async (queryText) => {
    ...
    try {
      ...
      if (hasUrls) {
        ...
        const failedScrapes = scrapeResults.filter(r => r.status === 'rejected')
        ...
        if (failedScrapes.length > 0) {
           const errMsg = failedScrapes.map(f => f.reason?.message || "Unknown error").join(" | ")
           addMessage({ role: 'assistant', content: `⚠️ Failed to scrape some URLs: ${errMsg}`, id: (Date.now() + 0.6).toString(), isError: true })
        }
      ...
      <AnimatePresence initial={false}>
        {isSidebarOpen && (
          <motion.div
            ...
            className="h-[calc(100vh-24px)] my-3 ml-3 bg-[#0c111d] border border-white/5 flex flex-col shrink-0 z-30 overflow-hidden whitespace-nowrap shadow-2xl rounded-[2rem] relative"
          >
            <div className="p-8 w-[280px] flex-1 overflow-y-auto custom-scrollbar">
              {/* Header */}
              <div className="flex items-start gap-4 mb-12 cursor-pointer group">
                <div className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-cyan-500 to-blue-600 flex items-center justify-center shadow-lg shadow-cyan-900/40 group-hover:shadow-cyan-500/50 transition-all group-hover:scale-110 relative overflow-hidden">
                  <div className="absolute inset-0 bg-white/10 opacity-0 group-hover:opacity-100 transition-opacity" />
                  <Sparkles size={24} className="text-white relative z-10" />
                </div>
                <div className="flex flex-col pt-0.5" onClick={() => setCurrentView('chat')}>
                  <span className="font-bold tracking-tight text-white group-hover:text-cyan-400 transition-colors text-xl leading-tight">Zero Memory</span>
                  <p className="text-[10px] text-cyan-500/80 font-bold uppercase tracking-[0.2em] mt-0.5">Workspace</p>
                </div>
                <button onClick={async (e) => { 
                  e.stopPropagation(); 
                  if(confirm("Wipe all system memory and start a fresh session?")) {
                    try { await resetMemory(); } catch (err) { console.error(err); }
                    useStore.setState({ messages: [], citations: null, trace: null, graphData: {nodes:[], links:[]}, uploadedFiles: [] }); 
                    setCurrentView('chat'); 
                  }
                }} className="absolute top-8 right-6 p-1.5 bg-white/5 hover:bg-red-500/20 hover:text-red-400 rounded-lg text-slate-500 transition-all opacity-0 group-hover:opacity-100">
              ...
              <div className="space-y-9">
                {/* Recent Sessions */}
                <div>
                  <p className="text-[9px] uppercase tracking-[0.2em] text-slate-500 font-bold mb-4 ml-1 flex items-center gap-2 opacity-80">
                    <MessageSquare size={12} className="text-slate-600" /> RECENT SESSIONS
                  </p>
                  <div className="space-y-0.5">
                    {recentSessions.length > 0 ? recentSessions.map((session, i) => (
                      <button key={i} onClick={() => setCurrentView('chat')} className="w-full flex items-center gap-3 px-3 py-2 rounded-xl transition-all hover:bg-white/5 text-[12px] text-slate-400 hover:text-slate-100 group overflow-hidden">
                        <div className="w-1 h-1 rounded-full bg-slate-700 group-hover:bg-cyan-500 transition-colors shrink-0" />
                        <span className="truncate flex-1 min-w-0">{session}</span>
                      </button>
                    )) : (
                      <p className="text-[11px] text-slate-600 italic px-3 py-1 opacity-50">No recent activity</p>
                    )}
                  </div>
                </div>
                ...
                <div>
                  <p className="text-[9px] uppercase tracking-[0.2em] text-slate-500 font-bold mb-4 ml-1 flex items-center gap-2 opacity-80">
                    <Library size={12} className="text-slate-600" /> DOCUMENTS
                  </p>
                  <div className="space-y-0.5">
                    {uniqueDocs.length > 0 ? uniqueDocs.map((doc, i) => (
                      <button key={i} className="w-full flex items-center gap-3 px-3 py-2 rounded-xl transition-all hover:bg-white/5 text-[12px] text-slate-400 hover:text-slate-100 group overflow-hidden">
                        <FileText size={13} className="text-slate-600 group-hover:text-blue-400 transition-colors shrink-0" />
                        <span className="truncate flex-1 min-w-0 text-left">{doc}</span>
                      </button>
                    )) : (
                      <p className="text-[11px] text-slate-600 italic px-3 py-1 opacity-50">No context loaded</p>
                    )}
                  </div>
                </div>
                ...
                <div>
                  <p className="text-[9px] uppercase tracking-[0.2em] text-slate-500 font-bold mb-4 ml-1 flex items-center gap-2 opacity-80">
                    <Settings size={12} className="text-slate-600" /> ENGINE CONFIGURATION
                  </p>
                  <div className="space-y-0.5">
                    <button onClick={() => setCurrentView('chat')} className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all text-[12px] text-left truncate group ${currentView === 'chat' ? 'bg-cyan-500/10 text-cyan-400 border border-cyan-500/10' : 'hover:bg-white/5 text-slate-400 hover:text-slate-100'}`}>
                      <MessageSquare size={14} className={`transition-colors shrink-0 ${currentView === 'chat' ? 'text-cyan-400' : 'text-slate-600 group-hover:text-cyan-400'}`} />
                      <span className="font-medium">Chat Interface</span>
                    </button>
                    <button onClick={() => setCurrentView('graph')} className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all text-[12px] text-left truncate group ${currentView === 'graph' ? 'bg-cyan-500/10 text-cyan-400 border border-cyan-500/10' : 'hover:bg-white/5 text-slate-400 hover:text-slate-100'}`}>
                      <Network size={14} className={`transition-colors shrink-0 ${currentView === 'graph' ? 'text-cyan-400' : 'text-slate-600 group-hover:text-cyan-400'}`} />
                      <span className="font-medium">Knowledge Graph</span>
                    </button>
                    <button onClick={() => setCurrentView('citations')} className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all text-[12px] text-left truncate group ${currentView === 'citations' ? 'bg-cyan-500/10 text-cyan-400 border border-cyan-500/10' : 'hover:bg-white/5 text-slate-400 hover:text-slate-100'}`}>
                      <BookOpen size={14} className={`transition-colors shrink-0 ${currentView === 'citations' ? 'text-cyan-400' : 'text-slate-600 group-hover:text-cyan-400'}`} />
                      <span className="font-medium">Citations</span>
                    </button>
                    <button onClick={() => setCurrentView('trace')} className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all text-[12px] text-left truncate group ${currentView === 'trace' ? 'bg-cyan-500/10 text-cyan-400 border border-cyan-500/10' : 'hover:bg-white/5 text-slate-400 hover:text-slate-100'}`}>
                      <Activity size={14} className={`transition-colors shrink-0 ${currentView === 'trace' ? 'text-cyan-400' : 'text-slate-600 group-hover:text-cyan-400'}`} />
                      <span className="font-medium">Agent Trace</span>
                    </button>
                    <button onClick={() => setCurrentView('upload')} className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all text-[12px] text-left truncate group ${currentView === 'upload' ? 'bg-cyan-500/10 text-cyan-400 border border-cyan-500/10' : 'hover:bg-white/5 text-slate-400 hover:text-slate-100'}`}>
                      <Database size={14} className={`transition-colors shrink-0 ${currentView === 'upload' ? 'text-cyan-400' : 'text-slate-600 group-hover:text-cyan-400'}`} />
                      <span className="font-medium">Ingest Management</span>
                    </button>
                  </div>
                </div>
              </div>
            ...
            {/* Profile Section */}
            <div className="mt-auto p-6 bg-[#090d16]/80 backdrop-blur-md border-t border-white/5">
              <div 
                onClick={() => setProfileOpen(!isProfileOpen)}
                className="flex items-center gap-4 cursor-pointer p-2.5 rounded-2xl hover:bg-white/5 transition-all group">
                <div className="w-11 h-11 rounded-full bg-[#050810] border border-white/10 flex items-center justify-center text-slate-300 shadow-xl group-hover:border-cyan-500/50 transition-all shrink-0">
                  <User size={18} />
                </div>
                <div className="flex-1 overflow-hidden">
                  <p className="text-sm font-bold text-slate-100 truncate">Alex Rivera</p>
                  <p className="text-[11px] text-slate-500 font-medium truncate mt-0.5">College Access Plan</p>
                </div>
                <Settings size={16} className="text-slate-600 group-hover:text-slate-300 transition-colors shrink-0" />
              </div>
            </div>
          </motion.div>
        )}
                      ...
                      {messages.map((msg) => (
                        ...
                        >
                          ...
                          <div className={`max-w-[85%] ${msg.role === 'user' ? 'bg-white/5 text-slate-200 px-5 py-3.5 rounded-2xl rounded-tr-sm border border-white/5 shadow-md shadow-black/20' : 'text-slate-300 pt-1'}`}>
                            {msg.role === 'assistant' ? (
                              <div className="prose prose-invert prose-lg prose-slate max-w-none leading-relaxed tracking-wide font-sans text-slate-200 break-words" style={{ overflowWrap: 'anywhere' }}>
                                <ReactMarkdown>{msg.content}</ReactMarkdown>
                              </div>
                            ) : (
                              <p className="leading-relaxed whitespace-pre-wrap text-xl font-medium break-words" style={{ overflowWrap: 'anywhere' }}>{msg.content}</p>
                            )}
                  ...
                  <div className="relative group">
                    ...
                    <div className="relative bg-[#0c111d] border border-white/10 rounded-lg shadow-2xl flex flex-col transition-all group-focus-within:border-cyan-500/50">
                      <textarea
                        ...
                        className="bg-transparent border-none focus:ring-0 text-slate-200 placeholder-slate-500 w-full min-h-[64px] max-h-52 resize-none text-lg focus:outline-none px-5 pt-4 pb-2 scrollbar-hide"
          ...
          {currentView === 'graph' && (
            <div className="flex-1 w-full h-full pt-16 relative z-10">
              <div className="absolute top-4 right-4 z-20 flex items-center gap-3">
                <div className="flex items-center gap-2 bg-[#050810]/50 backdrop-blur-md px-3 py-1.5 rounded-lg border border-white/5 text-cyan-400 text-sm font-medium tracking-widest uppercase">
                  <Network size={14} /> Knowledge Graph
                </div>
                <button onClick={() => setCurrentView('chat')} className="flex items-center gap-2 px-3 py-1.5 bg-[#0c111d]/80 hover:bg-[#151b2b] backdrop-blur-md rounded-lg border border-white/5 text-slate-300 text-sm transition-all shadow-lg hover:shadow-cyan-900/20 hover:text-cyan-400">
          ...
          {currentView === 'citations' && (
            <div className="flex-1 w-full h-full pt-16 overflow-y-auto relative z-10 px-4 max-w-5xl mx-auto">
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-2 text-cyan-400 text-sm font-medium tracking-widest uppercase">
                  <BookOpen size={14} /> Citations & Sources
                </div>
                <button onClick={() => setCurrentView('chat')} className="flex items-center gap-2 px-4 py-2 bg-white/5 hover:bg-white/10 rounded-xl text-slate-300 text-sm transition-all border border-white/5 hover:border-cyan-500/30 hover:text-cyan-400 shadow-md">
                  <MessageSquare size={14} /> Back to Chat
                </button>
              </div>
              <CitationPanel />
